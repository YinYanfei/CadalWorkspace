...
public class ReMaxMatchImprove extends Word{
		...
		while(position > 0){
			...
			if(ma.find()){
				...
				if(!this.segEnWord.SearchFun(str))  // 匹配这个来判断是否为is、am等等的单词
					this.arrWord.add(0, str); 			// 完全是英语时直接添加并跳过本次循环
	...
	public static void main(String[] args) {
		ReMaxMatchImprove reMaxMatchImp = new ReMaxMatchImprove();
		/*
		reMaxMatchImp.setSentence("浙江大学计算机学院的张志坤");
		reMaxMatchImp.setSentence("分布式全文搜索引擎的研究与实现");
		reMaxMatchImp.setSentence("毕业设计论文开题报告");
		reMaxMatchImp.setSentence("金融财务高新技术等企业的信息系统中" +
		 		  "数据存储的安全问题敏感数据的防窃取和防篡改问题越" +
		 		  "来越引起人们的重视");
		
		reMaxMatchImp.setSentence("随着计算机网络的发展数据库的安全性" +
		...
		*/
		reMaxMatchImp.setSentence("伴随着各行各业对数据库更加广泛的应" +
				  ...
				  "0");
		/*
		reMaxMatchImp.setSentence("而网友对于韩红的行为却褒贬不一有人" +
				  "支持她的率性也有人认为作为一个公众人物行为不应该" +
				  "如此偏激韩红表示我根本不看评论我只痛快我自己");
		*/
		
		// reMaxMatchImp.setSentence("中华人民共和国中央人民政府今天成立了");
		
		// Test Run-Time
		long startTime = System.currentTimeMillis();
		
		// Division
		reMaxMatchImp.Division();
