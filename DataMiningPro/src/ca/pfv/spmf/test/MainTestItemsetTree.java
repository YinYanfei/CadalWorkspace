...
import ca.pfv.spmf.algorithms.frequentpatterns.itemsettree.HashTable;
...
public class MainTestItemsetTree {
	...
	public static void main(String [] arg) throws IOException{
		...
		HashTable result = itemsetTree.getFrequentItemsetSubsuming(new int[]{1, 2});
		...
		int minsup = 2;
		HashTable result2 = itemsetTree.getFrequentItemsetSubsuming(new int[]{1}, minsup);
...
//					checkResult(itemset);   // THIS IS FOR DEBUGING  ONLY, IGNORE THIS LINE
		...
		minsup = 2;
		double minconf = 0.1;
		List<AssociationRule> rules = itemsetTree.generateRules(new int[]{1}, minsup, minconf);
