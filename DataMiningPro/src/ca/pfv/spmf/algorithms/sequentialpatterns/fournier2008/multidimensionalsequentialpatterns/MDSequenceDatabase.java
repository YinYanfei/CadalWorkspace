...
import ca.pfv.spmf.algorithms.sequentialpatterns.fournier2008.Item;
import ca.pfv.spmf.algorithms.sequentialpatterns.fournier2008.ItemValued;
import ca.pfv.spmf.algorithms.sequentialpatterns.fournier2008.Itemset;
...
public class MDSequenceDatabase {
	...
	private  final Set<Item> itemIDs = new HashSet<Item>();
	 ...
	 */
	public void processMDSequence(String[] tokens) {	
		...
		for(i++ ;i< tokens.length; i++){
			...
			if(tokens[i].codePointAt(0) == '<'){
				...
				if(indexLeftParenthesis != -1){
					...
					Item item = new Item(Integer.parseInt(tokens[i]));
