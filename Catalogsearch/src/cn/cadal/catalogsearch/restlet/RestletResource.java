...
public class RestletResource extends Resource {
	...
	private static final String defaultIndexDirPath = "/home/cadal/CodeRepository/cadal.indices/catalog";
	private static String indexDirPath = defaultIndexDirPath;
	private static final int defaultCount = 10;
	private static final int defaultStartNo = 0;
	...
	private static Hashtable<String, Float> fieldBoost = new Hashtable<String, Float>();

	static {
		try {
			String configLocation = String.format("%s/config.properties", RestletResource.class.getPackage().getName().replace('.', '/'));
			InputStream inputStream = RestletResource.class.getClassLoader().getResourceAsStream(configLocation);
			if (inputStream != null) {
				Properties p = new Properties();
				p.load(inputStream);
				indexDirPath = p.getProperty("indexdir", defaultIndexDirPath);
			}
	...
	private int startNo = defaultStartNo;
	private String type;
	...
	public Representation getRepresentation(Variant variant) {
		// do search
		IndexSearcher indexSearcher = !type.equals("paper") ? bookIndexSearcher : paperIndexSearcher;
		TopDocs topDocs = null;
		if (query != null && !query.equals(""))
			try {
				QueryParser queryParser = new QueryParser(Version.LUCENE_24, "Chapters", new ChineseAnalyzer());
				String queryString = QueryGenerator.getQuery(fieldBoost, query, new RelativeWords(query).getTermBoost());
				Query searchQuery = queryParser.parse(queryString);
				topDocs = indexSearcher.search(searchQuery, 100);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		// get representation
		...
		if (variant.getMediaType().equals(MediaType.TEXT_PLAIN)) {
			...
			if (topDocs != null) {
				...
				for (ScoreDoc scoreDoc : topDocs.scoreDocs)
					try {
						...
						String chapterEntry = getChapterEntry(doc.get("Chapters"), query);
						...
						resultBuilder.append(String.format("BookNo:%s\n"
								...
								+ "Publisher:%s\nSubject:%s\n"
								+ "Chapter:%s\nFileID:%s\n", doc.get("BookNo")
								.replace('\n', '\0'), doc.get("Title").replace(
								'\n', '\0'), doc.get("Creator").replace('\n',
								'\0'),
								doc.get("Publisher").replace('\n', '\0'), doc.get("Subject").replace('\n', '\0'), chapter, fileId));
						if (--left == 0)
							break;
					} catch (Exception e) {
						e.printStackTrace();
			...
			StringRepresentation stringRepresentation = new StringRepresentation(resultBuilder.toString());
			...
			JsonRepresentation jsonRepresentation = new JsonRepresentation(resultObject);
			if (jsonCallback != null && !jsonCallback.trim().equals("")) {
				jsonRepresentation.setText(jsonCallback + "(" + jsonRepresentation.getText() + ")");
			}
	...
	private static class QueryGenerator {
		public static String getQuery(Hashtable<String, Float> fields, String term, Hashtable<String, Float> rterms) {
			StringBuilder query = new StringBuilder();
			query.append("+(");
			for (Map.Entry<String, Float> fieldEntry : fields.entrySet())
				query.append(String.format("%s:\"%s\"^%f ", fieldEntry.getKey(), term, fieldEntry.getValue().floatValue()));
			query.append(") ");
			if (rterms.size() > 0)
				for (Map.Entry<String, Float> fieldEntry : fields.entrySet()) {
					query.append(String.format("%s:(", fieldEntry.getKey()));
					for (Map.Entry<String, Float> termEntry : rterms.entrySet())
						query.append(String.format("%s^%f ", termEntry.getKey(), termEntry.getValue().floatValue() * fieldEntry.getValue().floatValue()));
					query.append(") ");
				}
