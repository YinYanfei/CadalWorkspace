...
public class PersonalShare {
	...
	private byte[] cl_user_s = null;
	private byte[] cl_share_id = null;
	private byte[] cl_time = null;

	public PersonalShare() {
		this.handler = new Handler();
		this.htable = this.handler.HTableHandler(HBaseTable.PERSONAL_SHARE);
		this.cf = Bytes.toBytes("share");
		this.cl_ip = Bytes.toBytes("ip");
		this.cl_user = Bytes.toBytes("user");
		this.cl_user_s = Bytes.toBytes("user_s");
		this.cl_share_id = Bytes.toBytes("shareid");
		this.cl_time = Bytes.toBytes("time");
	}

	/**
	 * Insert single row into table HBaseTable.PERSONAL_SHARE
	 * 
	 * @param ip
	 * @param user
	 * @param user_s
	 ...
	 * @return
	 */
	public boolean insertSingleRow(String ip, String user, String user_s, String shareid, long timestamp) {
		try {
			...
			put.add(this.cf, this.cl_user_s, Bytes.toBytes(user_s));
			put.add(this.cf, this.cl_share_id, Bytes.toBytes(shareid));
	 ...
	 *        [
	 *            [
	 *            'ip':'10.15.62.75','user':'Yanfei','user_s':'zju','shareid':,'12hy71627h62hs62hw67','time':'1234567890'
	 *            ],
	 *            [
	 *            'ip':'10.15.62.75','user':'Yanfei','user_s':'zju','shareid':,'12hy71627h62hs62hw67','time':'1234567890'
	 *            ],
	...
	public boolean insertMultiRow(List<Map<String, String>> listRowInfo) {
		try {
			...
			for(Map<String,String> rowInfo: listRowInfo) {
				...
				put.add(this.cf, this.cl_user_s, Bytes.toBytes(rowInfo.get("user_s")));
				put.add(this.cf, this.cl_share_id, Bytes.toBytes(rowInfo.get("shareid")));
	...
	public static void main(String[] args) {
		// TEST
		PersonalShare personalShare = new PersonalShare();
		personalShare.Scanner("2000-01-01 00:00:00", "2013-12-03 21:00:00","C:\\Users\\hongxin\\Desktop\\test.out");
	//	personalShare.insertSingleRow("10.15.62.117", "Yanfei", "zju", "5452435464562tsy", System.currentTimeMillis());
