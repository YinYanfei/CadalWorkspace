...
public class ReadCommentOp {
	...
	private byte[] cl_pageno = null;
	private byte[] cl_comment = null;
	private byte[] cl_time = null;

	public ReadCommentOp() {
		this.handler = new Handler();
		this.htable = this.handler.HTableHandler(HBaseTable.READ_COMMENT);
		this.cf = Bytes.toBytes("comment");
		...
		this.cl_comment = Bytes.toBytes("comment");
	...
	public boolean insertSingleRow(String ip, String user, String bookid,
			String pageno, String comment, long timestamp) {
		try {
			...
			put.add(this.cf, this.cl_comment, Bytes.toBytes(comment));
	 ...
	 * @param listRowInfo
	 *            [ [
	 *            'ip':'10.15.62.75','user':'Yanfei','bookid':'07018720','pageno':'0000001','comment':'my
	...
	public boolean insertMultiRow(List<Map<String, String>> listRowInfo) {
		try {
			...
			for (Map<String, String> rowInfo : listRowInfo) {
				...
				put.add(this.cf, this.cl_comment,
						Bytes.toBytes(rowInfo.get("comment")));
	...
	public static void main(String[] args) {
		// TEST
		ReadCommentOp readComment = new ReadCommentOp();
		readComment.Scanner("2012-01-01 00:00:00", "2013-12-31 20:00:00","C:\\Users\\hongxin\\Desktop\\test.out");
		// readComment.insertSingleRow("10.15.62.110", "Yanfei", "07018720",
		// "00000002", "my comment", System.currentTimeMillis());
	}
