 ...
 * @author CADAL
 */
public class PersonalFollowSpout extends BaseRichSpout {
	...
	private void KafkaInit(){
		...
		props.put("group.id", "PersonalFollow");
		
		ConsumerConfig consumerConfig = new ConsumerConfig(props);
		ConsumerConnector consumer = kafka.consumer.Consumer.createJavaConsumerConnector(consumerConfig);

		Map<String, Integer> topicCountMap = new HashMap<String, Integer>();
		topicCountMap.put("Personal-follow", new Integer(1));// 第二个参数是指用几个流，多个流是为了并行处理。

		Map<String, List<KafkaStream<byte[],byte[]>>> consumerMap = consumer.createMessageStreams(topicCountMap);

		KafkaStream<byte[], byte[]> stream = consumerMap.get("Personal-follow").get(0);// 这里只有一个流，所以得get(0)就可以了。
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("follow"));		
	}
