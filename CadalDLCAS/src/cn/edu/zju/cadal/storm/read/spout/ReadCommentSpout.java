...
/**
 * 接收用户评论书页的信息，连接Kafka的consumer与storm的spout，topic名称为：Read-comment
 * 字段：ip user bookid pageno comment time
 */
public class ReadCommentSpout extends BaseRichSpout {
	...
	private void KafkaInit(){
		...
		props.put("group.id", "ReadComment");
		
		ConsumerConfig consumerConfig = new ConsumerConfig(props);
		ConsumerConnector consumer = kafka.consumer.Consumer.createJavaConsumerConnector(consumerConfig);

		Map<String, Integer> topicCountMap = new HashMap<String, Integer>();
		topicCountMap.put("Read-comment", new Integer(1));// 第二个参数是指用几个流，多个流是为了并行处理。

		Map<String, List<KafkaStream<byte[],byte[]>>> consumerMap = consumer.createMessageStreams(topicCountMap);

		KafkaStream<byte[], byte[]> stream = consumerMap.get("Read-comment").get(0);// 这里只有一个流，所以得get(0)就可以了。
