...
public class StreamingDSGD {
	public static void main(String[] args) throws Exception {
		if (args.length < 6) {
			...
			System.err.println("######## required args: local|production numUsers numItems inputFilename userOutputFilename itemOutputFilename");
			return;
		}

		Properties props = new Properties(); // 属性集（键值对）
		File propFile = new File("/home/Yanfei/Recommendation/collabstream.properties");
		...
		long inputDelay = Long.parseLong(props.getProperty("inputDelay", "50"));
		boolean debug = Boolean.parseBoolean(props.getProperty("debug", "false"));

		Configuration config = new Configuration(numUsers, numItems, numLatent,
				numUserBlocks, numItemBlocks, userPenalty, itemPenalty,
				initialStepSize, maxTrainingIters, inputFilename,
				userOutputFilename, itemOutputFilename, inputDelay, debug); // dsgd的配置
		...
		builder.setBolt("2", new Master(config))
				.globalGrouping("1")								// from 'spout- 1'
				.globalGrouping("3", Worker.TO_MASTER_STREAM_ID)	// from 'bolt - 3'
				.directGrouping("4")								// from 'bolt - 4'
				.directGrouping("5");								// from 'bolt - 5'
		
		// Userblocks那么多个worker(这概念和storm中的worker不同)
		builder.setBolt("3", new Worker(config), config.getNumWorkers())
				.fieldsGrouping("2", new Fields("userBlockIdx"))	// from 'bolt - 2'
				.directGrouping("4")								// from 'bolt - 4'
				.directGrouping("5");								// from 'bolt - 5'
		
		builder.setBolt("4", new MatrixStore(config), config.numUserBlocks)
				.fieldsGrouping("3", Worker.USER_BLOCK_STREAM_ID, new Fields("userBlockIdx"));    // from 'bolt - 3'
		
		builder.setBolt("5", new MatrixStore(config), config.numItemBlocks)
				.fieldsGrouping("3", Worker.ITEM_BLOCK_STREAM_ID, new Fields("itemBlockIdx"));    // from 'bolt - 3'
