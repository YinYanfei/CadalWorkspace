...
public class Worker implements IRichBolt {
	...
	public static final String TO_MASTER_STREAM_ID = "1";
	public static final String USER_BLOCK_STREAM_ID = "2";
	public static final String ITEM_BLOCK_STREAM_ID = "3";
	
	private OutputCollector collector;
	private final Configuration config;
	private final Map<BlockPair, WorkingBlock> workingBlockMap = new ConcurrentHashMap<BlockPair, WorkingBlock>();
	//映射blockpair和workingBlock
	...
	public void execute(Tuple tuple) {
		...
		case TRAINING_EXAMPLE:
			...
			workingBlock = getWorkingBlock(bp); //通过所在block映射到workingblock
			workingBlock.examples.add(ex);
			collector.ack(tuple);
			break;
		case PROCESS_BLOCK_REQ:
			bp = (BlockPair)tuple.getValue(1);
			workingBlock = getWorkingBlock(bp);	 //样本存储在workingblock中的		
			workingBlock.waitingForBlocks = true;
			//向matrixstore请求相应的block
	...
	private void update(BlockPair bp, WorkingBlock workingBlock) {
		...
		for (TrainingExample ex : examples) {
			...
			++ex.numTrainingIters;
			float stepSize = 2 * config.initialStepSize / ex.numTrainingIters; //计算梯度下降的步长，迭代次数越多步长越短
			//更新向量ui和vj
			for (int k = 0; k < config.numLatent; ++k) {
