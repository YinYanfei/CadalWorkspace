...
public class Master implements IRichBolt {
	...
	public void execute(Tuple tuple) {
		MsgType msgType = (MsgType)tuple.getValue(0);	// get the symbol of stream's information - 'new Field(msgType, example)' in spout
		...
		case END_OF_DATA:
			...
			break;
			//处理训练样本
		case TRAINING_EXAMPLE:  			// from spout
			ex = (TrainingExample)tuple.getValue(1);
			int userBlockIdx = config.getUserBlockIdx(ex.userId); //计算用户所在的blockid
			int itemBlockIdx = config.getItemBlockIdx(ex.itemId); //计算物品的blockid
			
			latest = latestExample[userBlockIdx][itemBlockIdx];
			if (latest == null || latest.timestamp < ex.timestamp) {
				latestExample[userBlockIdx][itemBlockIdx] = ex; //让latestEx中始终存储每个block中最新的样本
			}
			
			bp = blockPair[userBlockIdx][itemBlockIdx];
			if (bp == null) { //如果某个block是第一次出现
				bp = blockPair[userBlockIdx][itemBlockIdx] = new BlockPair(userBlockIdx, itemBlockIdx);
				unfinished.add(bp); //把这个block加入未完成block集合中
				freeSet.add(bp); //把这个block加入自由集中
			}
			...
			distributeWork();
			break;
		case PROCESS_BLOCK_FIN:				// from work bolt
			...
			if (latest.timestamp == ex.timestamp) {
				...
				if (endOfData && latest.numTrainingIters >= config.maxTrainingIters) {
					unfinished.remove(bp); //每个block中的latest样本达到最大迭代次数并且endofdata为真，该block停止处理
				}
			}
			
			// numTrainingIters must be updated before free() is called to prevent finished blocks
			// from being added to the freeSet
			free(bp.userBlockIdx, bp.itemBlockIdx); //解锁当前block所在的行和列
			
			if (unfinished.isEmpty()) {
				startOutput();
			} else {
				distributeWork(); //如果还有没到训练次数的block，就继续分配任务
			}
			break;
		case USER_BLOCK:					// from MatrixStoreUser
			bp = (BlockPair)tuple.getValue(1);
			float[][] userBlock = (float[][])tuple.getValue(2);
			head = userBlockQueue.remove(); //取出队首
			if (!head.equals(bp)) {
			...
			break;
		case ITEM_BLOCK:					// from MatrixStoreItem
			bp = (BlockPair)tuple.getValue(1);
	...
	private void lock(int userBlockIdx, int itemBlockIdx) { //锁定已分配block[i][j]所在的行和列
		for (int j = 0; j < config.numItemBlocks; ++j) {
			BlockPair bp = blockPair[userBlockIdx][j];
			if (bp != null) {
				freeSet.remove(bp); //把所在行的其他有样本的block从freeset中去掉
			}
		}
		for (int i = 0; i < config.numUserBlocks; ++i) {
			BlockPair bp = blockPair[i][itemBlockIdx];
			if (bp != null) {
				freeSet.remove(bp);//把所在列的其他有样本的block从freeset中去掉
			}
		}
	}
	...
	private void distributeWork() { //在自由集里随机选任务派发给worker
		...
		while (!freeSet.isEmpty()) {
			...
			lock(bp.userBlockIdx, bp.itemBlockIdx); //锁定被选中的block所在行和列
			collector.emit(new Values(PROCESS_BLOCK_REQ, bp, null, bp.userBlockIdx));//发送给worker处理
			freeList.clear();
		}
	}
	...
	private void startOutput() {
		try {
			...
			System.out.println("######## Elapsed training time: " + DurationFormatUtils.formatPeriod(startTime, outputStartTime, "H:m:s") + " (h:m:s)");
			...
			for (int i = 0; i < config.numUserBlocks; ++i) {
				...
				for (int j = 0; j < config.numItemBlocks; ++j) {
					...
					if (bp != null) {
						userBlockQueue.add(bp);  //找到每一行中第一个不为空的block放入userblockqueue中
						break;
					}
				}
			}
