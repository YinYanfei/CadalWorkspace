...
public class RatingsSource implements IRichSpout {
	...
	public void nextTuple() {
		...
		try {
			...
			if (line == null) {//输入完毕
				...
				System.out.println("######## Elapsed input time: " + DurationFormatUtils.formatPeriod(inputStartTime, inputEndTime, "H:m:s") + " (h:m:s)");
				
				input.close();
				input = null;
				collector.emit(new Values(END_OF_DATA, null), END_OF_DATA);				// emit -> 'END_OF_DATA'
			} else {
				try {
					String[] token = StringUtils.split(line, ' ');		//获得一个样本
					int userId = Integer.parseInt(token[0]);
					int itemId = Integer.parseInt(token[1]);
					float rating = Float.parseFloat(token[2]);
					
					// Bind these four type information
					TrainingExample ex = new TrainingExample(sequenceNum++, userId, itemId, rating);
					collector.emit(new Values(TRAINING_EXAMPLE, ex), ex);				// emit -> 'TRAINING_EXAMPLE'
																						// 向默认stream中发出tuple和messageid
	...
	public void declareOutputFields(OutputFieldsDeclarer declarer) {
		declarer.declare(new Fields("msgType", "example")); //声明spout发出去的tuple中的fields
	}
