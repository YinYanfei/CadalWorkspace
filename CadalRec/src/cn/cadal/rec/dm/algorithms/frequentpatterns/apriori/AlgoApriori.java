package cn.cadal.rec.dm.algorithms.frequentpatterns.apriori;
...
import cn.cadal.rec.dm.patterns.itemset_array_integers_with_count.Itemset;
import cn.cadal.rec.dm.patterns.itemset_array_integers_with_count.Itemsets;
import cn.cadal.rec.dm.tools.MemoryLogger;
 ...
 * memory if no output path is provided by the user.
 * 
...
public class AlgoApriori {
	 ...
	 *             file
	 */
	public Itemsets runAlgorithm(int minsup, String input, String output)
			throws IOException {
...
//		this.minsupRelative = (int) Math.ceil(minsup * databaseSize);  // if minsup is a percent of double type
	...
	/**
	 * Return the number of transaction in the last database read by the
	 * algorithm.
	 * 
	 * @return
	 */
